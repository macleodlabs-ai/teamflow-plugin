...
| `policy` | the same sidecar | `{action, reason ≤ 120, at, input, criticality, escalated, waitingOn?}` or null | One answer for WS-E's self-heal and WS-D's Attention row alike; `escalated` is whether the row is drawn or the entry stays in History. |
