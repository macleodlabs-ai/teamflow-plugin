...
TeamFlow is the DevOps harness for agentic software delivery. This plugin is its local half. It shows leads every agent at work in real time, against Jira, Linear or GitHub Issues, and it fixes stalled, orphaned and crashed agents. It sends derived state only, so your code stays on your machine. Normal use requires **zero TeamFlow commands**.
