...
`teamflowMcp` is TeamFlow's own MCP server, the `teamflow` entry in `/mcp`: doctor connects to it with the same header `/mcp` uses and says how many tools it offers, or why it is not connected. "Run /teamflow:login" there means this machine is not authorized; after logging in, Reconnect `teamflow` in `/mcp`. Enabling, disabling and reconnecting it are `/mcp`'s own actions.
